...
static bool check_mac_perms(pid_t spid, const char *tctx, const char *perm, const char *name)
{
    ...
    if (getpidcon(spid, &sctx) < 0) {
        fprintf(stderr, "SELinux: getpidcon(pid=%d) failed to retrieve pid context.\n", spid);
        return false;
    }
...
static bool check_mac_perms_from_lookup(pid_t spid, const char *perm, const char *name)
{
    ...
    if (!sehandle) {
        fprintf(stderr, "SELinux: Failed to find sehandle. Aborting service_manager.\n");
        abort();
    }

    if (selabel_lookup(sehandle, &tctx, name, 0) != 0) {
        fprintf(stderr, "SELinux: No match for %s in service_contexts.\n", name);
        return false;
    }
...
uint32_t do_find_service(struct binder_state *bs, const uint16_t *s, size_t len, uid_t uid, pid_t spid)
{
    ...
    if (!svc_can_find(s, len, spid)) {
        fprintf(stderr, "find_service('%s') uid=%d - PERMISSION DENIED\n",
             str8(s, len), uid);
        return 0;
    }
    si = find_svc(s, len);
    //fprintf(stderr, "check_service('%s') handle = %x\n", str8(s, len), si ? si->handle : 0);
    if (si && si->handle) {
                   ...
                   pid_t spid)
{
    ...
    //fprintf(stderr, "add_service('%s',%x,%s) uid=%d\n", str8(s, len), handle,
    ...
    if (!svc_can_register(s, len, spid)) {
        fprintf(stderr, "add_service('%s',%x) uid=%d - PERMISSION DENIED\n",
                   ...
                   struct binder_io *reply)
{
    ...
    //fprintf(stderr, "target=%x code=%d pid=%d uid=%d\n",
    ...
    case SVC_MGR_LIST_SERVICES: {
        ...
        if (!svc_can_list(txn->sender_pid)) {
            fprintf(stderr, "list_service() uid=%d - PERMISSION DENIED\n",
                    txn->sender_euid);
            return -1;
        }
        si = svclist;
...
int _serviceManager_loop(void)
{
    ...
    if (selinux_enabled > 0) {
        if (sehandle == NULL) {
            fprintf(stderr, "SELinux: Failed to acquire sehandle. Aborting.\n");
            abort();
        }

        if (getcon(&service_manager_context) != 0) {
            fprintf(stderr, "SELinux: Failed to acquire service_manager context. Aborting.\n");
            abort();
        }
    }
