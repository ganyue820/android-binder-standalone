    ...
    while (ptr < end) {
        ...
        case BR_TRANSACTION: {
            ...
            if ((end - ptr) < sizeof(*txn)) {
                fprintf(stderr, "svcmgr:parse: txn too small!\n");
                return -1;
            }
            binder_dump_txn(txn);
            if (func) {
        ...
        case BR_REPLY: {
            struct binder_transaction_data *txn = (struct binder_transaction_data *) ptr;
            if ((end - ptr) < sizeof(*txn)) {
                fprintf(stderr, "svcmgr:parse: reply too small!\n");
                return -1;
            }
            binder_dump_txn(txn);
            if (bio) {
        ...
        case BR_DEAD_REPLY:
            ...
            break;
        default:
            fprintf(stderr, "svcmgr:parse: OOPS %d\n", cmd);
            return -1;
        }
    }
...
void binder_loop(struct binder_state *bs, binder_handler func)
{
    ...
    for (;;) {
        ...
        if (res < 0) {
            fprintf(stderr, "svcmgr:binder_loop: ioctl failed (%s)\n", strerror(errno));
            break;
        }

        res = binder_parse(bs, 0, (uintptr_t) readbuf, bwr.read_consumed, func);
        if (res == 0) {
            fprintf(stderr, "svcmgr:binder_loop: unexpected reply?!\n");
            break;
        }
        if (res < 0) {
            fprintf(stderr, "svcmgr:binder_loop: io error %d %s\n", res, strerror(errno));
            break;
        }
    }
}
